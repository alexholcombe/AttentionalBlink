...
dM<-merge( select(df,-efficacy,-latency,-precision, -val), estimates_M )
...
g<-g+geom_histogram(binwidth=1,color="grey90",border=NA) + xlim(minSPE,maxSPE)
...
fontSz = 80/numGroups
g<-g + geom_text(data=fits_R_MATLAB,aes(x=-9,y=32, label = paste("-LogLik==", round(val,1), sep = "")),  parse = TRUE,size=fontSz) + 
  geom_text(data=fits_R_MATLAB,aes(x=-7,y=28, label = paste("plain(e)==", round(efficacy,2), sep = "")),  parse = TRUE,size=fontSz) +
  geom_text(data=fits_R_MATLAB,aes(x=-7,y=25, label = paste("mu==", round(latency,2), sep = "")),  parse = TRUE,size=fontSz)+
  geom_text(data=fits_R_MATLAB,aes(x=-7,y=22, label = paste("sigma==", round(precision,2), sep = "")),  parse = TRUE,size=fontSz) 
show(g)
```
